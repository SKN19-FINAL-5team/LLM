#!/usr/bin/env python3
"""
기준 데이터 Golden Set 생성 스크립트

실제 데이터베이스에서 기준 데이터를 샘플링하여
...
class CriteriaGoldenSetGenerator:
    ...
    def extract_query_from_chunk(self, content: str, item_name: str = None) -> str:
        ...
        # 품목 관련 키워드
        product_keywords = ['냉장고', '세탁기', '에어컨', 'TV', '스마트폰', '휴대폰', '노트북', 
                          '컴퓨터', '청소기', '전자레인지', '밥솥', '정수기', '공기청정기']
        found_products = [p for p in product_keywords if p in content_clean]
        if found_products:
            query_parts.extend(found_products[:2])
        
        # 기준 관련 키워드
        criteria_keywords = ['내용연수', '품질보증', '수리', '교환', '환불', '기준', '기간']
    ...
    def find_related_chunks(self, query: str, source_chunk_id: str, top_k: int = 10) -> List[str]:
        ...
        try:
            ...
            # doc_type이 'criteria_'로 시작하는 것만 필터링 및 source_chunk_id 제외
            related_chunk_ids = []
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='기준 데이터 Golden Set 생성')
    parser.add_argument('--num-samples', type=int, default=20,
                       help='생성할 샘플 수 (기본값: 20)')
    parser.add_argument('--output', type=str, default='golden_set_criteria.json',
                       help='출력 파일 경로 (기본값: golden_set_criteria.json)')
