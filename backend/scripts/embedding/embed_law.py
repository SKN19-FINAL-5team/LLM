...
법령 데이터(doc_type='law')만 처리하는 임베딩 스크립트
"""
...
class LawEmbeddingPipeline(EmbeddingPipeline):
    ...
    def process_all_files(self, data_dir: Path = None):
        ...
        print("\n" + "=" * 80)
        print("📚 법령 데이터 임베딩 파이프라인")
        print("=" * 80)
        print(f"데이터 디렉토리: {data_dir}")
        print(f"필터: doc_type = 'law'")
    ...
    def process_jsonl_file(self, jsonl_file: Path):
        ...
        if not documents:
            print("⚠️  법령 문서가 없습니다. 스킵.")
            return
        
        print(f"📊 로드된 법령 문서: {len(documents)}개")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='법령 데이터 임베딩 파이프라인')
    ...
    print("=" * 80)
    if args.load_only:
        print("📥 법령 데이터 로드만 수행 (임베딩 제외)")
    else:
        print("🚀 법령 데이터 임베딩 파이프라인 시작")
    print("=" * 80)
