#!/usr/bin/env python3
"""
피해구제 사례 데이터 임베딩 파이프라인

피해구제 사례 데이터(doc_type='consumer_counsel_case' 또는 'counsel_case')만 처리하는 임베딩 스크립트
"""
...
class CompensationEmbeddingPipeline(EmbeddingPipeline):
    """피해구제 사례 데이터만 처리하는 임베딩 파이프라인"""
    
    def process_all_files(self, data_dir: Path = None):
        """피해구제 사례 데이터 파일만 처리"""
        if data_dir is None:
            data_dir = DATA_DIR / "compensation_case"
        
        print("\n" + "=" * 80)
        print("💰 피해구제 사례 데이터 임베딩 파이프라인")
        print("=" * 80)
        print(f"데이터 디렉토리: {data_dir}")
        print(f"필터: doc_type IN ('consumer_counsel_case', 'counsel_case')")
    ...
    def process_jsonl_file(self, jsonl_file: Path):
        ...
        with open(jsonl_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    if doc_type in ('consumer_counsel_case', 'counsel_case'):
        ...
        if not documents:
            print("⚠️  피해구제 사례 문서가 없습니다. 스킵.")
            return
        
        print(f"📊 로드된 피해구제 사례 문서: {len(documents)}개")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='피해구제 사례 데이터 임베딩 파이프라인')
    ...
    print("=" * 80)
    if args.load_only:
        print("📥 피해구제 사례 데이터 로드만 수행 (임베딩 제외)")
    else:
        print("🚀 피해구제 사례 데이터 임베딩 파이프라인 시작")
    print("=" * 80)
    ...
    # 파이프라인 실행
    try:
        pipeline = CompensationEmbeddingPipeline(db_config, embed_api_url, load_only=args.load_only)
