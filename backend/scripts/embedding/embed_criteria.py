...
분쟁조정 기준 데이터(doc_type LIKE 'criteria_%')만 처리하는 임베딩 스크립트
"""
...
class CriteriaEmbeddingPipeline(EmbeddingPipeline):
    """기준 데이터만 처리하는 임베딩 파이프라인"""
    
    def process_all_files(self, data_dir: Path = None):
        """기준 데이터 파일만 처리"""
        if data_dir is None:
            data_dir = DATA_DIR / "criteria"
        
        print("\n" + "=" * 80)
        print("📋 기준 데이터 임베딩 파이프라인")
        print("=" * 80)
        print(f"데이터 디렉토리: {data_dir}")
        print(f"필터: doc_type LIKE 'criteria_%'")
    ...
    def process_jsonl_file(self, jsonl_file: Path):
        ...
        with open(jsonl_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    if doc_type.startswith('criteria_'):
        ...
        if not documents:
            print("⚠️  기준 문서가 없습니다. 스킵.")
            return
        
        print(f"📊 로드된 기준 문서: {len(documents)}개")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='기준 데이터 임베딩 파이프라인')
    ...
    print("=" * 80)
    if args.load_only:
        print("📥 기준 데이터 로드만 수행 (임베딩 제외)")
    else:
        print("🚀 기준 데이터 임베딩 파이프라인 시작")
    print("=" * 80)
    ...
    # 파이프라인 실행
    try:
        pipeline = CriteriaEmbeddingPipeline(db_config, embed_api_url, load_only=args.load_only)
