...
class ComprehensiveCompensationRAGTester:
    """피해구제 사례 데이터 종합 RAG 테스터"""
    ...
    def __init__(self, db_config: Dict):
        ...
        # BM25 초기화 (피해구제 사례용은 BM25가 직접 지원하지 않으므로 벡터 검색 사용)
        # SPLADE 초기화
        if SPLADE_AVAILABLE:
            try:
    ...
    def search_cosine(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        compensation_results = [
            r for r in results 
            if r.get('source') in ('consumer_counsel_case', 'counsel_case')
        ]
        return compensation_results[:top_k]
    
    def search_bm25(self, query: str, top_k: int = 10) -> List[Dict]:
        """BM25 검색 (피해구제 사례는 BM25 직접 지원 없음, 벡터 검색 사용)"""
        # 피해구제 사례는 BM25가 직접 지원하지 않으므로 Cosine Similarity 사용
    ...
    def search_splade(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        try:
            # SPLADE는 피해구제 사례를 직접 지원하지 않을 수 있으므로
            # 벡터 검색 결과를 반환
            return self.search_cosine(query, top_k)
        except Exception as e:
    ...
    def test_query(self, query: str, methods: List[str], top_k: int = 10):
        ...
        for method, data in all_results.items():
            ...
            for i, result in enumerate(data['results'][:5], 1):
                ...
                print(f"     문서 타입: {result.get('source', 'N/A')}")
    ...
    def test_batch(self, golden_set_file: Path, methods: List[str], top_k: int = 10):
        ...
        print("\n" + "=" * 80)
        print("💰 피해구제 사례 데이터 배치 테스트 (Golden Set)")
        print("=" * 80)
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='피해구제 사례 데이터 종합 RAG 테스트')
    ...
    parser.add_argument('--golden-set', type=str, default='golden_set_compensation.json',
                       help='배치 모드에서 사용할 golden set 파일 경로 (기본값: golden_set_compensation.json)')
    ...
    # 테스터 초기화
    tester = ComprehensiveCompensationRAGTester(db_config)
