...
| S2-4 | **도메인 설정 세분화** | - 금융 분쟁조정(FSS, 금융감독원), 의료 분쟁조정(K-Medi, 한국의료분쟁조정중재원)<br>- [분쟁조정.md](docs/guides/분쟁조정.md) 기준 참고   | - Domain test Golden Set 생성(실 데이터 기반)<br>- Golden Set 기반 테스트 정확도 > 0.8 | AI/MAS Product Manager |
| S2-5 | **에이전트 평가 기준 수립 및 평가 시스템 구현** | - 에이전트별 평가 지표 정의 (하단 상세 참조)<br>- Golden Set 구축 (기존 DB 데이터 활용)<br>- CLI 기반 평가 스크립트 구현<br>- 평가 결과 JSON/CSV 리포트 출력 | - 각 에이전트별 평가 스크립트 실행 가능<br>- Golden Set 50+ 샘플 구축<br>- Retrieval nDCG@5 ≥ 0.65 달성 | AI/MAS System Engineer |
| S2-6 | **기관별 절차/양식 기반 가이드 생성** | - 추천 기관에 맞는 분쟁조정 절차 안내<br>- 기관별 제출 양식(템플릿)에 맞춘 사용자 안내 가이드 출력<br>- 기관별 양식 수집 전략 수립(KCA, KCDRC, ECMC 만 제공) | - 동일 입력에 대해 기관별 양식에 맞는 가이드 생성 | AI/MAS System Engineer |
| S2-7 | **Backend 서버화 및 배포(AWS EC2)** | - Chat Service Server + DB Server 분리 운영 전제의 구성 정리 <br>- Server Sent Event 구성 (Chat Stream)<br>- 환경변수/로그/헬스체크/보안(최소) 포함 배포 가이드 | - EC2에서 기본 채팅 플로우가 동작(검증 체크리스트) | Full-stack |
