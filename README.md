...
| S2-7 | **Backend 서버화 및 배포(AWS EC2) (Week 3)** | - SSE 스트리밍 구현 (`astream_events` + `EventSourceResponse`)<br>- Production Docker Compose + Nginx 설정 (SSE 버퍼링 비활성화)<br>- Gunicorn + UvicornWorker (4 workers) 프로덕션 구성<br>- Let's Encrypt SSL, 헬스체크/RotatingFileHandler 로깅 | - EC2에서 채팅 스트리밍 정상 동작<br>- 배포 체크리스트 100% 통과 | Full-stack |
...
Linux/macOS 환경에서는 백엔드가 자동으로 임베딩 서버를 시작합니다 (`embedding_connection.py`의 `start_local_server()`).

```bash
# 1. 환경 변수 설정 (최초 1회)
cp backend/.env.example backend/.env
# .env 파일에 실제 API Key 설정 필요
...
#### Windows 11 (PowerShell) - Docker 전체 실행 (권장)

Windows 환경에서는 Docker 네트워크 격리로 인해 임베딩 서버를 별도 컨테이너로 실행해야 합니다. `docker-compose.windows.yml`에는 임베딩 서비스가 포함되어 있습니다.
...
| embedding | 8001 | KURE-v1 임베딩 서버 (CPU 모드) |
| backend | 8000 | FastAPI 백엔드 |
| frontend | 5173 | React 프론트엔드 |
| cloudbeaver | 8978 | 웹 기반 DB 관리 도구 |
