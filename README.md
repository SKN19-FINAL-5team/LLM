...
### 1.1. 리포지토리 구조 (Repository Structure)

```bash
backend/
├── app/
│   ├── agents/                 # (New) 모듈화된 에이전트
│   │   ├── answer_generation/  # 답변 생성 에이전트 (구 rag/generator.py)
│   │   ├── legal_review/       # 법률 검토 에이전트
│   │   ├── query_analysis/     # 질의 분석 에이전트
│   │   ├── react/              # ReAct 추론/액션 노드
│   │   └── retrieval/          # 정보 검색 에이전트 (구 rag/retrievers)
│   ├── common/                 # 공통 유틸리티 (logger 등)
│   ├── orchestrator/           # LangGraph 그래프 및 상태 관리
│   └── main.py                 # FastAPI 애플리케이션 진입점
├── data/                       # 법령, 판례, 상담 등 원본 데이터
├── scripts/                    # 유틸리티, 평가, 통합 테스트 코드
```
...
│   ├── shared/                 # 공통 UI 컴포넌트 및 유틸리티
│   ├── store/                  # 전역 상태 관리
│   ├── widgets/                # 페이지를 구성하는 복합 위젯
...
| **Dense Embedding** | nlpai-lab/KURE-v1 | Dimension=1024 | 의미 기반 벡터 검색 (Semantic Search) |
| **Sparse Embedding** | BAAI/bge-m3 | - | 키워드 매칭 보완 (Dense + Lexical Match) |
...
### Sprint 2 — MAS 확장 + 서버화: 오케스트레이터 + 질의분석/검토 에이전트 + 배포 기반 (Week 2 ~ 3)

**Week 2 (Completed)**: 질의분석, 정보검색, 답변생성, 검토 에이전트, 오케스트레이터 구현 완료.
**Week 3 (Planned)**: 도메인 심화(평가), 가이드 생성, 서버 배포 및 최적화.

| PR | 목표 | 상세 범위(요약) | 완료 기준/데모 | 검토 Persona |
|:---|:---|:---|:---|:---|
| S2-1 | **질의분석 에이전트(형식화/라우팅)** (✅ 완료) | - 사용자 입력을 구조화(구매정보/분쟁유형/핵심 키워드/필터)<br>- KCA/ECMC/KCDRC 라우팅(초기 Rule-base + 점진적 LLM 보조) | - 동일 케이스에 대해 기관 추천이 일관되고 근거(규칙)가 표시 | AI/MAS System Engineer |
| S2-2 | **검토 에이전트(가드레일/근거성)** (✅ 완료) | - 최종 답변에 대해 “근거 없는 주장”, “법적 판단/단정”, “데이터 기반 아닌 내용” 탐지<br>- 필요 시 답변 수정/반환(근거 부족 시 ‘모름’ 처리) | - 할루시네이션/법적 단정 방지 체크리스트 통과 | Data Scientist |
| S2-3 | **오케스트레이터(LangGraph) 및 상태관리** (✅ 완료) | - 질의분석 → 검색 → 답변생성 → 검토의 워크플로우 정의<br>- 멀티턴 상태(온보딩 정보/추가 질문) 유지 | - 멀티턴 2~3회 대화에서 일관된 상태 기반 응답 | AI/MAS System Engineer |
| S2-4 | **도메인 설정 세분화 (Week 1)** | - 금융 분쟁조정(FSS, 금융감독원), 의료 분쟁조정(K-Medi, 한국의료분쟁조정중재원), 개인정보 분쟁조정(kopico)<br>- [분쟁조정.md](docs/guides/분쟁조정.md) 기준 참고   | - Domain test Golden Set 생성(실 데이터 기반)<br>- Golden Set 기반 테스트 정확도 > 0.8 | AI/MAS Product Manager |
| S2-5 | **에이전트 평가 기준 수립 및 시스템 구현 (Week 1-2)** | - 에이전트별 평가 지표 정의 (하단 상세 참조)<br>- Golden Set 구축 (기존 DB 데이터 활용)<br>- CLI 기반 평가 스크립트 구현<br>- 평가 결과 JSON/CSV 리포트 출력 | - 각 에이전트별 평가 스크립트 실행 가능<br>- Golden Set 50+ 샘플 구축<br>- Retrieval nDCG@5 ≥ 0.65 달성 | AI/MAS System Engineer |
| S2-6 | **기관별 절차/양식 기반 가이드 생성 (Week 2)** | - KCA/ECMC/KCDRC 기관별 JSON 템플릿 구축 (신청절차/필수서류/처리기간)<br>- `AgencyGuide` 데이터 모델 및 `/guide/generate` API 구현<br>- 분쟁 유형별 맞춤 서류 체크리스트 생성<br>- "다음 행동" 섹션에 가이드 통합 | - 3개 기관 가이드 템플릿 완성<br>- 공식 URL/연락처 정확성 검증 통과<br>- 답변 내 기관별 가이드 표시 | AI/MAS System Engineer |
| S2-7 | **ReAct Orchestrator 구축** | - State 스키마에 `thoughts`/`actions`/`observations` 필드 추가<br>- LangGraph ReAct 패턴 그래프 재설계<br>- 조건부 순환 엣지 구현 (max 2회 반복) | - 2회 반복 테스트 통과<br>- 중간 추론 단계 로깅 확인 | AI/MAS System Engineer |
| S2-8 | **EXAONE 3.5 2.4B 통합** | - EXAONE 3.5 2.4B RunPod 배포<br>- RunPod → Local GPU 폴백 로직 구현<br>- vLLM OpenAI-compatible 클라이언트<br>- Query Analysis에 LLM 추론 추가 | - RunPod/Local GPU 헬스체크 통과<br>- 폴백 전환 테스트 통과<br>- 한국어 분류 정확도 ≥0.85 | AI/MAS System Engineer |
| S2-9 | **BGE-M3 Sparse 통합 (Phase 2)** | - BGE-M3 Sparse 임베딩 파이프라인 추가<br>- Hybrid Retrieval에 Dense+Sparse 통합<br>- 기존 KURE Dense와 병행 사용 | - Sparse 검색 품질 검증<br>- 검색 지연시간 ≤100ms | AI/MAS System Engineer |
| S2-10 | **LLM 기반 쿼리 재작성 (Phase 3)** | - EXAONE 기반 쿼리 재작성<br>- HyDE 가상 문서 생성<br>- 컨텍스트 인식 확장 | - 쿼리 품질 향상 측정<br>- 지연시간 ≤300ms | AI/MAS System Engineer |
| S2-11 | **ReAct 통합 테스트** | - E2E 테스트 시나리오 작성<br>- Golden Set 기반 평가<br>- 추론 단계 시각화 | - 테스트 커버리지 ≥80%<br>- 추론 trace 저장 | AI/MAS System Engineer |
| S2-12 | **Backend 서버화 및 배포(AWS EC2)** | - SSE 스트리밍 구현 (`astream_events` + `EventSourceResponse`)<br>- Production Docker Compose + Nginx 설정 (SSE 버퍼링 비활성화)<br>- Gunicorn + UvicornWorker (4 workers) 프로덕션 구성<br>- Let's Encrypt SSL, 헬스체크/RotatingFileHandler 로깅 | - EC2에서 채팅 스트리밍 정상 동작<br>- 배포 체크리스트 100% 통과 | Full-stack |

## 4. 평가 전략

### 4.1. 에이전트별 평가 지표

#### 4.1.1. Query Analysis Agent

| 지표 | 설명 | 목표 |
|-----|------|-----|
| Query Type Accuracy | 질의 유형 분류 정확도 (dispute/general/law/criteria) | ≥ 0.90 |
| Keyword Precision | 추출 키워드 정밀도 | ≥ 0.80 |
| Keyword Recall | 추출 키워드 재현율 | ≥ 0.70 |
| Agency Hint Accuracy | 기관 추천 힌트 정확도 | ≥ 0.85 |
| Missing Field Detection F1 | 누락 필드 탐지 F1 | ≥ 0.85 |

#### 4.1.2. Information Retrieval Agent (RAGAS 기반)

| 지표 | 설명 | 목표 |
|-----|------|-----|
| Context Precision | 검색 컨텍스트 정밀도 (상위 K개 중 관련 청크 비율) | ≥ 0.70 |
| Context Recall | 검색 컨텍스트 재현율 (필요 정보 포함 비율) | ≥ 0.75 |
| nDCG@K | 순위 품질 (Normalized DCG, K=5,10) | ≥ 0.65 |
| MRR | 첫 관련 문서 순위 (Mean Reciprocal Rank) | ≥ 0.60 |
| Hit Rate@K | 상위 K개 내 관련 문서 존재 여부 | ≥ 0.85 |

**섹션별 세부 지표**: disputes(사례 관련성), laws(조문 정확성), criteria(품목 매칭률)

#### 4.1.3. Answer Generation Agent

| 지표 | 설명 | 목표 |
|-----|------|-----|
| Faithfulness | 근거 충실도 (근거 있는 주장 비율) | ≥ 0.90 |
| Answer Relevancy | 답변 관련성 (질문-답변 의미 유사도) | ≥ 0.80 |
| Citation Accuracy | 인용 정확도 (올바른 출처 인용 비율) | ≥ 0.95 |
| HPT (Hallucination Per Turn) | 턴당 환각 수 | ≤ 0.5 |
| Entity Grounding | 엔티티 근거성 (출처에 존재하는 엔티티 비율) | ≥ 0.90 |

> **참고**: 법률 도메인 특화 지표는 [HalluDetect](https://arxiv.org/html/2509.11619), [HalluGraph](https://arxiv.org/html/2512.01659) 연구 기반

#### 4.1.4. Legal Review Agent

| 지표 | 설명 | 목표 |
|-----|------|-----|
| Violation Detection Precision | 위반 탐지 정밀도 | ≥ 0.85 |
| Violation Detection Recall | 위반 탐지 재현율 | ≥ 0.90 |
| False Positive Rate | 오탐률 (정상 표현을 위반으로 판단) | ≤ 0.10 |
| Filter Effectiveness | 필터링 효과 (수정 후 위반 감소율) | ≥ 0.80 |

**검토 규칙**: 절대적 표현("반드시"), 법적 단정("위법입니다"), 예측 표현("승소할 것") 등 13개 패턴

### 4.2. Golden Set 구축 전략

```
backend/data/golden_set/
├── query_analysis/       # 100+ 샘플 (질의 유형별 균등 분포)
├── retrieval/            # 50+ 샘플 (기대 청크 매핑)
├── generation/           # 50+ 샘플 (참조 답변, 환각 사례)
├── review/               # 100+ 샘플 (위반/정상 표현)
└── e2e/                  # 30+ 시나리오 (통합 테스트)
```

- **데이터 소스**: 기존 DB의 분쟁조정사례/상담사례 활용
- **레이블링**: 전문가 교차 검증 (Cohen's Kappa ≥ 0.80 목표)

### 4.3. 평가 스크립트 (CLI)

```bash
# Retrieval Agent 평가 (우선 구현)
python -m scripts.evaluation.evaluate_retrieval --golden-set ./data/golden_set/retrieval

# Generation Agent 평가
python -m scripts.evaluation.evaluate_generation --golden-set ./data/golden_set/generation

# 전체 E2E 평가
python -m scripts.evaluation.evaluate_e2e --golden-set ./data/golden_set/e2e

# 리포트 생성
python -m scripts.evaluation.generate_report --output ./reports/
```

### 4.4. 서비스 KPI (출시/개선 지표)

- **라우팅 정확도(F1)**: KCA/ECMC/KCDRC/FSS/K-Medi 분류 F1 ≥ 0.85
- **근거 충실도**: Faithfulness ≥ 0.90 (RAGAS 기준)
- **사용자 행동 전환**: 체크리스트 저장률, 접수 링크 클릭률

### 4.5. 운영 지표 (SLO)

- **성능**: p95 응답시간 ≤ 10s, 스트리밍 시작 지연 ≤ 2s
- **안정성**: 실패율 ≤ 1%, 타임아웃 비율 ≤ 0.5%
- **비용**: 세션당 LLM 토큰 비용 모니터링

> **평가 프레임워크 참고**: [RAGAS](https://docs.ragas.io/en/stable/concepts/metrics/available_metrics/), [Langfuse](https://langfuse.com/guides/cookbook/example_langgraph_agents), [Promptfoo](https://www.promptfoo.dev/docs/guides/evaluate-langgraph/)

## 5. 실행 및 테스트 가이드 (Execution Guide)

상세한 실행 방법(RunPod 설정, Docker, DB, 서버 실행, 테스트 등)은 별도 가이드 문서를 참고해 주세요.

**[실행 및 테스트 가이드 보러가기](docs/guides/EASY_START_GUIDE_KR.md)**
