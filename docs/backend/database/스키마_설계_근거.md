...
erDiagram
    ...
    chunks {
        ...
        varchar chunk_type "조문/결정문/기준 등"
        text content
        vector embedding "1024차원 벡터"
        bool drop "검색 제외 플래그"
    }
...
```mermaid
graph TB
    ConsumerCase[피해구제상담사례<br/>cs_*.jsonl]
    Criteria[소비자분쟁해결기준<br/>table2_resolution]
    DisputeCase[분쟁조정사례<br/>kca/ecmc/kcdrc]
    Law[법령<br/>11개 법령]
    
    ConsumerCase -->|인용| Law
    Criteria -->|laws 참조| Law
    DisputeCase -->|판단근거| Law
    DisputeCase -->|적용| Criteria
...
```mermaid
graph TB
    Q[사용자 질의:<br/>란류 부패 환불]
    C1[상담사례:<br/>란류 부패 환불 Q&A]
    L1[법령:<br/>종자산업법 제23조]
    CR[기준:<br/>table2_resolution_row]
    D1[판례:<br/>KCA 2015일가27]
    D2[판례:<br/>ECMC 2016가123]
...
```mermaid
graph TB
    subgraph SeparateDBs[분리된 Vector DBs]
        DB1[법령 DB]
        DB2[기준 DB]
        DB3[분쟁조정 DB]
        DB4[상담사례 DB]
